...
CREATE TRIGGER `before_users_insert` BEFORE INSERT ON `users` FOR EACH ROW BEGIN
    -- If 'username' is provided and 'name' is NULL or empty, set 'name' to 'username'
    IF NEW.username IS NOT NULL AND NEW.username != '' AND (NEW.name IS NULL OR NEW.name = '') THEN
        SET NEW.name = NEW.username$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `before_users_update` BEFORE UPDATE ON `users` FOR EACH ROW BEGIN
    -- If 'username' is changed and 'name' was equal to the old 'username', update 'name' to new 'username'
    IF NEW.username != OLD.username AND OLD.username = OLD.name THEN
        SET NEW.name = NEW.username$$
DELIMITER ;
