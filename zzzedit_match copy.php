...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Debug datetime before saving
    echo "Normalized datetime (before save): $datetime\n";

    // Update match in the database
    $stmt = $conn->prepare("UPDATE matches 
        SET tournament_id = ?, category_id = ?, player1_id = ?, player2_id = ?, stage = ?, 
            match_time = ?, set1_player1_points = ?, set1_player2_points = ?, 
    ...
    $stmt->bind_param(
        ...
        $datetime, $set1_p1, $set1_p2, $set2_p1, $set2_p2, $set3_p1, $set3_p2, $match_id
    );
    ...
    // Debug saved data
    $query = $conn->prepare("SELECT match_time FROM matches WHERE id = ?");
    $query->bind_param("i", $match_id);
    $query->execute();
    $query->bind_result($saved_datetime);
    $query->fetch();
    echo "Saved Date-Time (from database): $saved_datetime\n";
    $query->close();
}
